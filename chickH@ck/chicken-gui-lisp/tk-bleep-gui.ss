;; So here is the chicken version of our bleep program. This time using the TK framework, which
;; I've used a lot with python. - Again this was by far the easiest one so far and my choice to 
;; work with as of right now. I'm still pending:
;; chicken5 with IUP
;; CL with GTK
;; ECL with EQL (QT)
(import pstk)
                ...
                (let loop ((i 0))
                  (when (< i samples-per-buffer)
                    (let ((time (/ (+ (* samples-per-buffer n) i) stream-frequency)))
                      ; al:audio-stream-fragment returns a C pointer. use (chicken memory)
                      ; module to operate on foreign pointer objects.
...
(tk-start)
(ttk/set-theme "aquativo")
(ttk-map-widgets 'all) ; Use the ttk widget set

; Create a spin box with a units  label
; Returns from widget encompassing both spin box and label and the spin box widget itself.
; This way we can access the value of the spin box. 
...
;; So far this has been BY FAR the easiest one to work with as of yet.
